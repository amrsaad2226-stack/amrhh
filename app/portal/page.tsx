...
export default async function EmployeePortal() {
  // 👈 التعديل هنا: استخدمنا await لجلب ملفات الارتباط
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 rounded-[2.5rem] p-8 text-white shadow-xl mb-8 flex justify-between items-center">
          ...
          <form action={logoutEmployee}>
            <button className="bg-white/20 hover:bg-white/30 p-3 rounded-2xl backdrop-blur-sm transition-all text-white">
              <LogOut size={24} />
            </button>
          </form>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div><p className="text-slate-400 text-xs font-bold">ساعات الإضافي</p><p className="text-2xl font-black text-slate-800">{totalOvertime} س</p></div>
          </div>
            ...
            <tbody>
              {employee.attendances.map(att => (
                <tr key={att.id} className="border-t border-slate-50 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="p-4"><span className="bg-amber-100 text-amber-700 px-2 py-1 rounded-lg text-xs font-bold">{att.overtime} س</span></td>
                </tr>
              ))}
            </tbody>
          </table>
