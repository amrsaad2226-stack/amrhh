...
import { History, Download, Archive, ChevronLeft, Calendar, FileClock, CheckCircle, XCircle, Clock, ChevronDown } from "lucide-react";
      ...
      {employee.leaveRequests && employee.leaveRequests.length > 0 && (
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <ChevronDown size={24} className={`text-slate-400 transition-transform duration-300 ${isLeaveHistoryOpen ? 'rotate-180' : ''}`} />
          </button>
          {isLeaveHistoryOpen && (
            <div className="p-4 border-t border-slate-100 dark:border-slate-800 space-y-3">
              {employee.leaveRequests.map((leave: any) => (
                <div key={leave.id} className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl flex justify-between items-center">
                  <div>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-100 dark:border-slate-800">
        <div className="flex items-center justify-between p-4">
          ...
          >
            ...
            سجل النشاط
            <ChevronDown size={20} className={`text-slate-400 transition-transform duration-300 ${isActivityLogOpen ? 'rotate-180' : ''}`} />
          </button>
          <div className="flex items-center gap-2">
            <button 
              onClick={exportToPDF} 
                ...
                {[...attendanceRecords].reverse().map((record: any) => (
                    <div key={record.id} className="relative flex gap-4 group">
                      ...
                      <div className="mr-8 flex-1 bg-slate-50 dark:bg-slate-800/50 p-5 rounded-[2rem] hover:shadow-md transition-all hover:-translate-y-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-slate-600 dark:text-slate-400 bg-white dark:bg-slate-900/50 p-3 rounded-2xl">
                        ...
                        {(record.isLastOfDay && record.checkOut) && (
                          <div className="mt-4 grid grid-cols-3 md:grid-cols-6 gap-x-1 gap-y-4 border-t border-slate-200 dark:border-slate-700 pt-4">
