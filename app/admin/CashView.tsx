...
import { DollarSign, PlusCircle, ArrowUp, ArrowDown, Trash2, Edit, Search, X, Calendar } from 'lucide-react';
...
// Function to get today's date in YYYY-MM-DD format for Cairo timezone
const getTodayDateString = () => {
    const today = new Date();
    // Adjust for Cairo's timezone (UTC+2 or UTC+3 depending on DST)
    const cairoOffset = today.getTimezoneOffset() + (180 * 60000); // EEST is UTC+3, so 180 minutes
    const cairoTime = new Date(today.getTime() + cairoOffset);
...
export default function CashView({ transactions, employees }: CashViewProps) {
  ...
  const handleFormSubmit = async (formData: FormData) => {
    ...
    if (result?.error) {
      ...
      form?.reset();
      // Reset the date input to today after successful submission
    ...
    <>
        <div id="cash-section" className="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-6 md:p-8 mb-10">
            ...
            <div className="max-w-2xl mx-auto mb-10">
                <form id="cash-transaction-form" action={handleFormSubmit} className="bg-slate-50/50 border rounded-2xl p-6 space-y-4">
                    <h3 className="font-bold text-center text-lg">تسجيل حركة جديدة</h3>
                    <input type="hidden" name="type" value={transactionType} />
                    {transactionType === 'INCOME' && <input type="hidden" name="incomeSource" value={incomeSource} />}

                    <div className="flex gap-4">
                        <div onClick={() => setTransactionType('OUTCOME')} className={`flex-1 text-center p-4 rounded-xl cursor-pointer border-2 transition-all ${transactionType === 'OUTCOME' ? 'bg-red-100 border-red-300' : 'bg-white hover:bg-slate-100'}`}><ArrowDown className="mx-auto mb-2 text-red-500" /><span className="font-bold text-red-600">سند صرف</span></div>
                        <div onClick={() => setTransactionType('INCOME')} className={`flex-1 text-center p-4 rounded-xl cursor-pointer border-2 transition-all ${transactionType === 'INCOME' ? 'bg-green-100 border-green-300' : 'bg-white hover:bg-slate-100'}`}><ArrowUp className="mx-auto mb-2 text-green-500" /><span className="font-bold text-green-600">سند قبض</span></div>
                    </div>

                    {transactionType === 'INCOME' && (
                        <div className="p-3 bg-white rounded-lg border">
                            <label className="text-sm font-bold text-slate-600 mb-2 block">مصدر الإيداع:</label>
                            <div className="flex gap-2">
                                <div onClick={() => setIncomeSource('treasury')} className={`flex-1 text-center p-2 text-sm rounded-md cursor-pointer border ${incomeSource === 'treasury' ? 'bg-blue-100 text-blue-700' : 'hover:bg-slate-100'}`}>من الخزنة</div>
                                <div onClick={() => setIncomeSource('employee')} className={`flex-1 text-center p-2 text-sm rounded-md cursor-pointer border ${incomeSource === 'employee' ? 'bg-blue-100 text-blue-700' : 'hover:bg-slate-100'}`}>سداد من موظف</div>
                            </div>
                        </div>
                    )}

                    <div className={`grid grid-cols-1 ${isEmployeeRequired ? 'md:grid-cols-2' : ''} gap-4`}>
                        <div>
                            <label htmlFor="amount" className="text-sm font-bold text-slate-600 mb-1 block">المبلغ</label>
                            <input id="amount" name="amount" type="number" step="0.01" required className="w-full p-3 border rounded-lg" />
                        </div>
                        {isEmployeeRequired && (
                            <div>
                                <label htmlFor="employeeId" className="text-sm font-bold text-slate-600 mb-1 block">الموظف</label>
                                <select id="employeeId" name="employeeId" required className="w-full p-3 border rounded-lg bg-white">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label htmlFor="note" className="text-sm font-bold text-slate-600 mb-1 block">البيان</label>
                            <textarea id="note" name="note" rows={2} required className="w-full p-3 border rounded-lg"></textarea>
                        </div>
                        <div>
                            <label htmlFor="transaction-date" className="text-sm font-bold text-slate-600 mb-1 block">تاريخ الحركة</label>
                            <input id="transaction-date" name="createdAt" type="date" defaultValue={getTodayDateString()} required className="w-full p-3 border rounded-lg" />
                        </div>
                    </div>

                    <button type="submit" disabled={isLoading} className={`w-full py-4 rounded-lg font-bold text-white transition-all flex items-center justify-center gap-2 ${isLoading ? 'bg-slate-400' : 'bg-green-600 hover:bg-green-700'}`}>
                        {isLoading ? "جاري الحفظ..." : <><PlusCircle size={20}/> حفظ الحركة</>}
                    </button>
                </form>
            </div>
            ...
            <div>
                ...
                <div className="bg-slate-50 p-4 rounded-xl mb-4 border grid md:grid-cols-3 gap-4 items-end">
                    <div className="md:col-span-1">
                        ...
                        <input type="text" id="search" placeholder="..." className="w-full p-2 border rounded-lg" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                    <div>
                        <label htmlFor="startDate" className="text-sm font-bold text-slate-600 mb-1">من</label>
                        <input type="date" id="startDate" className="w-full p-2 border rounded-lg" value={startDate} onChange={e => setStartDate(e.target.value)} />
                    </div>
                    <div>
                        <label htmlFor="endDate" className="text-sm font-bold text-slate-600 mb-1">إلى</label>
                        <input type="date" id="endDate" className="w-full p-2 border rounded-lg" value={endDate} onChange={e => setEndDate(e.target.value)} />
                    </div>
                </div>
                    ...
                    <table className="w-full text-right">
                        ...
                        {filteredTransactions.map(t => (
                            <tr key={t.id} className="hover:bg-slate-50">
                                <td className="p-4 text-sm text-slate-600">{new Date(t.createdAt).toLocaleDateString('ar-EG', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</td>
            ...
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-2xl shadow-xl w-full max-w-md">
                    <form action={handleUpdateSubmit} className="p-6 space-y-4">
                        ...
                        <div>
                            <label htmlFor="edit-date" className="text-sm font-bold text-slate-600 mb-1 block">تاريخ الحركة</label>
                            <input id="edit-date" name="createdAt" type="date" required className="w-full p-3 border rounded-lg bg-slate-50" defaultValue={new Date(editingTransaction.createdAt).toISOString().split('T')[0]} />
                        </div>
                        <div>
